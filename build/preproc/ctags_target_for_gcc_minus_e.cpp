...
static const TickType_t LED_DELAY_NORMAL = ( ( TickType_t ) ( ( ( TickType_t ) ( 30 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) );
...
# 314 "/home/chandrashekar/Arduino/esp32_haptic/esp32_haptic.ino"
    ...
    virtual bool ConsumeSample(int16_t sample[2]) override {
        ...
        esp_err_t err = i2s_channel_write(tx_chan, &scaled, 4, &bytes_written, ( ( TickType_t ) ( ( ( TickType_t ) ( 20 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ));
        if (err == 0 /*!< esp_err_t value indicating success (no error) */ && bytes_written >= 4) {
    ...
    virtual uint16_t ConsumeSamples(int16_t *samples, uint16_t count) override {
        ...
        while (processed < count) {
            ...
            size_t bytes_written = 0;
            esp_err_t err = i2s_channel_write(tx_chan, scaled, toProcess * 4, &bytes_written, ( ( TickType_t ) ( ( ( TickType_t ) ( 20 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ));
            if (err != 0 /*!< esp_err_t value indicating success (no error) */) {
                return processed;
            }
...
void _stopAudioSafely() {
    if (audioMutex) {
        ...
        xQueueSemaphoreTake( ( audioMutex ), ( ( TickType_t ) 0xffffffffUL ) );
    }
...
# 609 "/home/chandrashekar/Arduino/esp32_haptic/esp32_haptic.ino"
    ...
    *outSize = 0;
    ...
    if (!xQueueSemaphoreTake( ( sdMutex ), ( ( ( TickType_t ) ( ( ( TickType_t ) ( 350 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ) ) )) {
        return false;
    }

    bool ok = false;
    do {
        File src = SD_MMC.open(filename, "r");
        if (!src) break;

        size_t sz = src.size();
        if (sz == 0 || sz > (5U * 1024U * 1024U)) {
            src.close();
            break;
        }
...
bool _startAudioPlayback(const String& filename, bool loopRequested, uint32_t startByte, bool updateMediaState, uint32_t sessionId) {
    ...
    if (!updateMediaState) {
        ...
        do {
            ...
            if (!id3Source || !mp3) {
                Serial0.println("Failed to create companion decoder pipeline");
                break;
            }

            if (!mp3->begin(id3Source, out)) {
                Serial0.println("Failed to start companion MP3 decoder");
                break;
            }
            ...
            isPaused = false;
            success = true;
            Serial0.println("Companion MP3 started from PSRAM");
        } while (false);
...
# 741 "/home/chandrashekar/Arduino/esp32_haptic/esp32_haptic.ino"
    ...
    bool success = false;
    do {
        Serial0.printf("Opening media file: %s\n", filename.c_str());
...
# 771 "/home/chandrashekar/Arduino/esp32_haptic/esp32_haptic.ino"
        ...
        Serial0.println("Starting decoder begin()...");
        if (!mp3->begin(id3Source, out)) {
...
bool _enqueueAudioCommand(const AudioCommandMessage &cmd, uint32_t timeoutMs) {
    ...
    return xQueueGenericSend( ( audioCmdQueue ), ( &cmd ), ( ( ( TickType_t ) ( ( ( TickType_t ) ( timeoutMs ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ) ), ( ( BaseType_t ) 0 ) ) == ( ( BaseType_t ) 1 );
}
...
void _resetVideoFrameQueues() {
    ...
    if (videoDisplayQueue) {
        VideoPacket packet;
        while (xQueueReceive(videoDisplayQueue, &packet, 0) == ( ( BaseType_t ) 1 )) {
...
void taskAudio(void *pvParameters) {
    ...
    while (true) {
        ...
        while (xQueueReceive(audioCmdQueue, &cmdMsg, 0) == ( ( BaseType_t ) 1 )) {
        ...
        if (isUploading) {
            vTaskDelay(( ( TickType_t ) ( ( ( TickType_t ) ( 20 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ));
            continue;
        }
...
# 1086 "/home/chandrashekar/Arduino/esp32_haptic/esp32_haptic.ino"
                   , ( ( TickType_t ) 0U ), ( ( BaseType_t ) 0 ) );
                    ...
                    if ((now - lastAudioMutexLogMs) >= 1000U) {
                        Serial0.println("WARN: Audio mutex busy; skipping pump cycle");
        ...
        TickType_t audioDelay = (currentMediaType == MEDIA_VIDEO && videoCompanionAudioActive) ? ( ( TickType_t ) ( ( ( TickType_t ) ( 5 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ) : ( ( TickType_t ) ( ( ( TickType_t ) ( 2 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) );
...
void taskLEDs(void *pvParameters) {
    ...
    while (true) {
        if (isUploading) {
            vTaskDelay(( ( TickType_t ) ( ( ( TickType_t ) ( 100 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ));
            continue;
        }
...
void initDisplayVideoScaffold() {
    ...
    tftMutex = xQueueCreateMutex( ( ( uint8_t ) 1U ) );
    tftSpi = new SPIClass(1 /* ESP32S2, S3, P4 - SPI 3 bus*/);
    if (tftSpi) {
        tftSpi->begin(18, -1, 17, 16);
...
# 1203 "/home/chandrashekar/Arduino/esp32_haptic/esp32_haptic.ino"
    ...
    videoMjpegFrameCapacity = (300U * 1024U);
    videoMjpegFrameData = (uint8_t*)ps_malloc(videoMjpegFrameCapacity);
    videoMjpegDecodeBufferSize = max(videoRawFrameSize, (size_t)(320U * 320U) * 2U);
...
# 1217 "/home/chandrashekar/Arduino/esp32_haptic/esp32_haptic.ino"
    ...
    if (displayReady) {
        ...
        Serial0.println("ST7789 panel init OK (RGB test shown)");
    } else {
        Serial0.println("WARN: ST7789 panel init failed");
    }
    Serial0.println("Video scaffold initialized (decode/display active).");
}

void st7789WriteCommand(uint8_t command, const uint8_t *data, size_t dataLen) {
    if (!tftSpi) return;
    if (tftMutex && xQueueSemaphoreTake( ( tftMutex ), ( ( ( TickType_t ) ( ( ( TickType_t ) ( 20 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ) ) ) != ( ( BaseType_t ) 1 )) return;

    tftSpi->beginTransaction(SPISettings(40000000, 1, 0));
    digitalWrite(16, 0x0);
    digitalWrite(15, 0x0);
...
void st7789FillColor(uint16_t color) {
    ...
    if (tftMutex && xQueueSemaphoreTake( ( tftMutex ), ( ( ( TickType_t ) ( ( ( TickType_t ) ( 30 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ) ) ) != ( ( BaseType_t ) 1 )) return;
    ...
    uint16_t x1 = 0 + 240 - 1;
    uint16_t y0 = 20;
    uint16_t y1 = 20 + 280 - 1;

    digitalWrite(15, 0x0); tftSpi->transfer(0x2A);
    digitalWrite(15, 0x1);
    ...
    digitalWrite(15, 0x0); tftSpi->transfer(0x2B);
    digitalWrite(15, 0x1);
    ...
    digitalWrite(15, 0x0); tftSpi->transfer(0x2C);
    digitalWrite(15, 0x1);
...
void st7789DrawFrameRGB565(const uint8_t *frameData, uint16_t width, uint16_t height) {
    ...
    if (width != 240 || height != 280) return;
    if (tftMutex && xQueueSemaphoreTake( ( tftMutex ), ( ( ( TickType_t ) ( ( ( TickType_t ) ( 40 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ) ) ) != ( ( BaseType_t ) 1 )) return;

    tftSpi->beginTransaction(SPISettings(40000000, 1, 0));
    digitalWrite(16, 0x0);

    uint16_t x0 = 0;
    uint16_t x1 = 0 + width - 1;
    uint16_t y0 = 20;
    uint16_t y1 = 20 + height - 1;
...
bool _startVideoPlayback(const String& filename, bool loopRequested, uint32_t sessionId) {
    ...
    if (lower.endsWith(".v16") || lower.endsWith(".rgb")) {
        ...
        if (!videoUseMjpegStream) {
            Serial0.println("Video MJPEG stream open failed.");
            videoStart_state = VIDEO_START_FAILED;
            return false;
        }
        ...
        if (!videoUseFileStream) {
            Serial0.println("Video .vid stream open failed.");
            videoStart_state = VIDEO_START_FAILED;
            return false;
        }
    } else {
        Serial0.println("Unsupported video extension.");
        videoStart_state = VIDEO_START_FAILED;
        return false;
    }
...
bool _openVideoColorStream(const String& filename) {
    ...
    if (!xQueueSemaphoreTake( ( sdMutex ), ( ( ( TickType_t ) ( ( ( TickType_t ) ( 300 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ) ) )) {
        return false;
    }
    ...
    do {
        ...
        videoStreamFile = SD_MMC.open(filename, "r");
        if (!videoStreamFile) {
            break;
        }
...
bool _openRawFrameStream(const String& filename) {
    ...
    do {
        ...
        videoRawFile = SD_MMC.open(filename, "r");
        ...
        if (videoRawWidth != 240 || videoRawHeight != 280 || videoRawFps == 0) {
            videoRawFile.close();
            break;
        }
...
static bool _decodeJpegToFrameBuffer(const uint8_t *jpegData, size_t jpegSize, uint8_t frameIndex, bool displayNative = false) {
    ...
    if (esp_jpeg_get_image_info(&probeCfg, &probeOut) != 0 /*!< esp_err_t value indicating success (no error) */ || probeOut.width == 0 || probeOut.height == 0) {
        return false;
    }

    esp_jpeg_image_scale_t scale = _selectJpegScale(probeOut.width, probeOut.height);
    const bool rotateCw = (0 != 0);
    const bool rotateCcw = (1 != 0);
    const bool needsRotate = rotateCw || rotateCcw;
    const bool needsTransform = needsRotate || 0 || 0;
    ...
    if (esp_jpeg_decode(&decodeCfg, &decodeOut) != 0 /*!< esp_err_t value indicating success (no error) */ || decodeOut.width == 0 || decodeOut.height == 0) {
        return false;
    }
    ...
    if (1) {
        ...
        uint32_t srcMul = (uint32_t)drawWidth * (uint32_t)280;
        uint32_t dstMul = (uint32_t)240 * (uint32_t)drawHeight;
        if (srcMul > dstMul) {
            cropW = (uint16_t)(((uint32_t)drawHeight * (uint32_t)240) / (uint32_t)280);
            cropX = (uint16_t)((drawWidth - cropW) / 2U);
        } else if (srcMul < dstMul) {
            cropH = (uint16_t)(((uint32_t)drawWidth * (uint32_t)280) / (uint32_t)240);
            cropY = (uint16_t)((drawHeight - cropH) / 2U);
        }
    } else {
        if (outW > 240 || outH > 280) {
            return false;
        }
        dstX = (uint16_t)((240 - outW) / 2U);
        dstY = (uint16_t)((280 - outH) / 2U);
    }
    ...
    for (uint16_t y = 0; y < outH; y++) {
        for (uint16_t x = 0; x < outW; x++) {
            ...
            size_t dstIndex = ((size_t)(dstY + y) * (size_t)240 + (size_t)(dstX + x)) * 2U;
            if (1) {
                target[dstIndex] = hi;
...
bool _openMjpegStream(const String& filename) {
    ...
    do {
        ...
        videoMjpegFile = SD_MMC.open(filename, "r");
...
bool _readMjpegFrameBytes(size_t *outFrameLen) {
    ...
    *outFrameLen = 0;
    ...
    uint8_t chunk[8192];
    _videoSdReadBegin();
    if (!xQueueSemaphoreTake( ( sdMutex ), ( ( ( TickType_t ) ( ( ( TickType_t ) ( 60 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ) ) )) {
        _videoSdReadEnd();
        return false;
    }
...
# 2225 "/home/chandrashekar/Arduino/esp32_haptic/esp32_haptic.ino"
    ...
    if (!exists) {
        ...
        videoCompanionAudioFile = "";
        videoCompanionAudioStartMs = 0;
        Serial0.println("No companion .mp3 found for video");
        return;
    }
    ...
    if (_startAudioPlayback(audioFile, loopRequested, 0, false, activeSessionId)) {
        ...
        videoCompanionAudioFile = "";
        videoCompanionAudioStartMs = 0;
        Serial0.printf("Companion audio failed: %s\n", audioFile.c_str());
    }
}
...
void setup() {
    ...
    FastLED.addLeds<WS2812B, 40, GRB>(leds, 24).setCorrection(TypicalLEDStrip);
    ...
    if (!SD_MMC.setPins(12, 11, 13)) {
        Serial0.println("SD_MMC pin configuration failed!");
    }

    if (!SD_MMC.begin("/sdcard", true)) {
        Serial0.println("SD_MMC initialization failed!");
    } else {
        Serial0.println("SD_MMC initialized.");
    }

    sdMutex = xQueueCreateMutex( ( ( uint8_t ) 1U ) );
    audioMutex = xQueueCreateMutex( ( ( uint8_t ) 1U ) );
    if (!audioMutex) {
        Serial0.println("WARN: Failed to create audio mutex");
    }
    audioCmdQueue = xQueueGenericCreate( ( 8 ), ( sizeof(AudioCommandMessage) ), ( ( ( uint8_t ) 0U ) ) );
    if (!audioCmdQueue) {
        Serial0.println("ERROR: Failed to create audio command queue");
    }
 ...
 send_ota_ack(otaConfig, payload); //send boot ack with device info
    ...
    server.on("/api/media/upload", HTTP_POST, [](){
        ...
        while (!uploadProcessingDone && (millis() - waitStartMs) < 15000U) {
            vTaskDelay(( ( TickType_t ) ( ( ( TickType_t ) ( 2 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ));
        }
...
void loop() {
    vTaskDelay(( ( TickType_t ) ( ( ( TickType_t ) ( 1000 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ));
}
...
void handleApiFiles() {
    ...
    TickType_t waitTicks = (currentMediaType == MEDIA_VIDEO) ? ( ( TickType_t ) ( ( ( TickType_t ) ( 15 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ) : ( TickType_t ) 0xffffffffUL;
    if (xQueueSemaphoreTake( ( sdMutex ), ( waitTicks ) )) {
...
void handleApiCurrent() {
    ...
    if (currentMediaType == MEDIA_AUDIO) {
        ...
        if (audioMutex) {
            audioLocked = (xQueueSemaphoreTake( ( audioMutex ), ( ( ( TickType_t ) ( ( ( TickType_t ) ( 20 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ) ) ) == ( ( BaseType_t ) 1 ));
            if (!audioLocked) {
                uint32_t now = millis();
                if ((now - lastAudioStatusLockLogMs) >= 1000U) {
                    Serial0.println("WARN: Audio mutex busy during status read");
...
void handleApiMediaDelete() {
    ...
    TickType_t waitTicks = (currentMediaType == MEDIA_VIDEO) ? ( ( TickType_t ) ( ( ( TickType_t ) ( 20 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ) : ( TickType_t ) 0xffffffffUL;
    if (xQueueSemaphoreTake( ( sdMutex ), ( waitTicks ) )) {
...
void handleApiMediaStorage() {
    ...
    if (xQueueSemaphoreTake( ( sdMutex ), ( ( ( TickType_t ) ( ( ( TickType_t ) ( 120 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ) ) ) != ( ( BaseType_t ) 1 )) {
...
const TickType_t UPLOAD_SD_LOCK_WAIT_TICKS = ( ( TickType_t ) ( ( ( TickType_t ) ( 120 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) );
const TickType_t UPLOAD_QUEUE_SEND_WAIT_TICKS = ( ( TickType_t ) ( ( ( TickType_t ) ( 1000 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) );
...
uint32_t _readFileSizeWithRetriesLocked(const String &path, uint8_t retries = 4) {
    ...
    for (uint8_t i = 0; i < retries; i++) {
        if (!path.length() || !SD_MMC.exists(path)) {
            if (i < retries - 1) {
                vTaskDelay(( ( TickType_t ) ( ( ( TickType_t ) ( 5 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ));
            }
            continue;
        }
...
size_t _writeUploadChunkLocked(const uint8_t *data, size_t len) {
    ...
    while (xQueueSemaphoreTake( ( sdMutex ), ( UPLOAD_SD_LOCK_WAIT_TICKS ) ) != ( ( BaseType_t ) 1 )) {
        lockRetries++;
...
bool _ensureUploadWorker() {
    ...
    if (!uploadWorkQueue) {
        uploadWorkQueue = xQueueGenericCreate( ( UPLOAD_QUEUE_DEPTH ), ( sizeof(UploadWorkItem) ), ( ( ( uint8_t ) 0U ) ) );
        if (!uploadWorkQueue) {
...
bool _enqueueUploadWorkRaw(const void *itemPtr, const char *context) {
    ...
    if (xQueueGenericSend( ( uploadWorkQueue ), ( itemPtr ), ( UPLOAD_QUEUE_SEND_WAIT_TICKS ), ( ( BaseType_t ) 0 ) ) != ( ( BaseType_t ) 1 )) {
...
void taskUploadWorker(void *pvParameters) {
    ...
    while (true) {
        if (!uploadWorkQueue || xQueueReceive(uploadWorkQueue, &item, ( TickType_t ) 0xffffffffUL) != ( ( BaseType_t ) 1 )) {
            vTaskDelay(( ( TickType_t ) ( ( ( TickType_t ) ( 2 ) * ( TickType_t ) 1000 ) / ( TickType_t ) 1000U ) ));
            continue;
        }
        ...
        if (item.type == UPLOAD_WORK_START) {
            ...
            if (uploadFile) {
                if (xQueueSemaphoreTake( ( sdMutex ), ( UPLOAD_SD_LOCK_WAIT_TICKS ) ) == ( ( BaseType_t ) 1 )) {
                    uploadFile.close();
                    uploadFile = File();
                    xQueueGenericSend( ( QueueHandle_t ) ( sdMutex ), 
...
# 3628 "/home/chandrashekar/Arduino/esp32_haptic/esp32_haptic.ino"
            ...
            if (!uploadWriteScratch) {
                uploadWriteScratch = (uint8_t*)heap_caps_malloc(UPLOAD_WRITE_CHUNK_SIZE, (1<<11) /*|< Memory must be internal; specifically it should not disappear when flash/spiram cache is switched off*/ | (1<<2) /*|< Memory must allow for 8/16/...-bit data accesses*/);
                if (!uploadWriteScratch) {
            ...
            if (xQueueSemaphoreTake( ( sdMutex ), ( UPLOAD_SD_LOCK_WAIT_TICKS ) ) == ( ( BaseType_t ) 1 )) {
                ...
                if (SD_MMC.exists(uploadTargetPath)) {
                    File existing = SD_MMC.open(uploadTargetPath, "r");
                    if (existing) {
                ...
                if (!uploadHadWriteError) {
                    uploadFile = SD_MMC.open(uploadPartialPath, "w");
                }
...
# 3768 "/home/chandrashekar/Arduino/esp32_haptic/esp32_haptic.ino"
            ...
            isUploading = false;
            if (uploadWorkQueue) {
                xQueueGenericReset( ( uploadWorkQueue ), ( ( BaseType_t ) 0 ) );
            }
            Serial0.println("Upload Aborted by client!");
        }
    }
}
...
# 3947 "/home/chandrashekar/Arduino/esp32_haptic/esp32_haptic.ino"
        ...
        if (uploadFile && xQueueSemaphoreTake( ( sdMutex ), ( UPLOAD_SD_LOCK_WAIT_TICKS ) ) == ( ( BaseType_t ) 1 )) {
            uploadFile.close();
            uploadFile = File();
            if (uploadPartialPath.length() && SD_MMC.exists(uploadPartialPath)) {
                SD_MMC.remove(uploadPartialPath);
